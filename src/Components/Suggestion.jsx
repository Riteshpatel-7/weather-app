...
export default function Suggestion({ input, setCityData, setSuggestions, setMyLocalSuggestion }) {
    ...
    if (input === '')
        ...
        return <>{
            (citiesData === undefined) ? <Navigate to="/error?message=Error occurred while fetching data. Try with different input or check your internet connection." replace /> :
            ...
            {citiesData.map((city, index) => (
                <div onClick={(e) => {setCityData(getCityDataObj(e.target.id)); setSuggestions(false); setMyLocalSuggestion(false) }} id={index} key={index} className='bg-transparent  p-2 rounded-lg hover:border hover:border-[hsl(243,23%,30%)] hover:bg-[hsl(243,23%,24%)]'>{city.name}, {city.admin1}, {city.country}</div>
            ))}
        </div>} 
        </>
    }
